    ...
    const char *updated, const char *html_body)
{
    ...
    fputs("<script>\n"
          ...
          "      var rows=filtered.map(function(it){return [it.id||'',it.createdAt||'',it.editor||'',it.ip||'',it.saveTxnId||'',it.contentLength||0].join('\\t');}).join('\\n');\n"
          "      var removed=items.length-filtered.length;\n"
          "      var txt='\\u6587\\u7ae0 ID\\uff1a'+id+'\\n\\u603b\\u8bb0\\u5f55\\uff1a'+filtered.length+(removed>0?'\\uff08\\u5df2\\u8fc7\\u6ee4 '+removed+' \\u6761\\u76f8\\u540c\\u7248\\u672c\\uff09':'')+'\\n\\n#\\t\\u4fdd\\u5b58\\u65f6\\u95f4\\t\\u7f16\\u8f91\\u8005\\tIP\\tsave_txn_id\\t\\u5185\\u5bb9\\u957f\\u5ea6\\n'+rows;\n"
