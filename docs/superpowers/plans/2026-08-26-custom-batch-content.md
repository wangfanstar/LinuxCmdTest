...
已通过生成 HTML 运行时脚本解析与状态依赖检查；当前浏览器策略不允许以 `data:` URL 打开临时生成页，因此未对浏览器会话写入网络库文件。
