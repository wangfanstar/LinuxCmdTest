...
assert.deepStrictEqual(demotionDocument.groups.map(group => group.id), ['group-alpha', 'group-gamma']);
...
assert.deepStrictEqual(demotion.item.children.map(item => item.id), ['beta-root']);
...
assert.deepStrictEqual(nestedPromotionDocument.groups.map(group => group.title), ['甲组', '乙组', '乙子', '丙组']);
...
assert.deepStrictEqual(nestedPromotionDocument.groups[1].items[0].children.map(item => item.id), []);
...
assert.deepStrictEqual(topPromotionDocument.groups.map(group => group.title), ['甲组', '乙组', '乙根', '丙组']);
