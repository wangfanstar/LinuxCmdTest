...
- HTML 文件显示文件名、大小和更新时间，提供“预览”和“新窗口打开”。预览使用受限静态 URL 加载 iframe；打开使用新标签页。文件名经过 URL 编码，不拼接用户可控路径。
...
- `POST /api/html-paste/save`：JSON 请求体 `{ name, content, overwrite }`；仅允许 `.json`，默认同名返回 `409`，`overwrite:true` 才覆盖；写入采用临时文件再重命名，避免半写入文件。

静态文件服务继续提供 `/html_paste/<name>.html`，并由现有 `..` 路径检查和新增文件名校验共同保证安全。接口不开放删除、任意目录写入或 HTML 写入，降低本地服务暴露在局域网时的风险。
...
HTML 预览区默认使用 `sandbox="allow-scripts allow-forms allow-modals"`，不授予顶层导航权限；“新窗口打开”才以普通页面打开，便于查看完整交互。列表为空时显示目录为空提示。
...
本次不实现网络库删除、重命名、目录层级、用户权限系统或独立生成 HTML 页面访问服务器 API；独立页面继续只支持本地 JSON 导入导出。
